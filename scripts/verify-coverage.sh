...
for case in "${CASES[@]}"; do
    ...
    out=$(docker run --rm curlimages/curl:latest -s -m 30 -XPOST \
        --data "[[$lon,$lat]]" "http://$IP:8080/v1/elevations")
    got=$(printf '%s' "$out" | tr -d ' []')
    if [ "$expect" = "null" ]; then
        [ "$got" = "null" ] && r="ok" || { r="FAIL"; fail=1; }
    else
        [ "$got" != "null" ] && [ -n "$got" ] && r="ok" || { r="FAIL"; fail=1; }
    fi
    printf '  %-4s %-16s %-10s expected %s\n' "$r" "$label" "$got" "$expect"
done
