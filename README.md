...
|---|---|---|
| 2025年版全臺灣20公尺網格數值地形模型DTM資料 | [176927](https://data.gov.tw/dataset/176927) | `不分幅_全台20MDEM(2025).zip`<br>`不分幅_澎湖20MDEM(2025).zip`<br>`不分幅_金門20MDEM(2025).zip` |

> 索引中的**圖資名稱**與**連結網址**裡的檔名不一定相同：本島的顯示名稱是
> 「不分幅_台灣20MDEM(2025)」，實際檔名卻是 `不分幅_全台20MDEM(2025).zip`。
> 澎湖與金門兩者一致，所以只驗其中一個是驗不出來的。取檔一律以連結網址為準，
> `make check-urls` 會逐一確認三個網址都還通。
